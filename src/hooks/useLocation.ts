...
export function useCountries() {
  ...
  const fetchCountries = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      logger.userAction('Fetching countries list');
      
      const response = await apiService.getCountryList();
      
      if (response.status === 1) {
        setCountries(response.data);
        logger.info('Countries fetched successfully', { count: response.data.length });
      } else {
...
// Combined hook for location selection
