...
import { useLocationSelection } from '../hooks/useLocation';
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      {selectedCountry && (
        <div className="form-group">
          {showLabels && (
            <label className="form-label">
              Estado/Província {required && <span className="text-red-500">*</span>}
            </label>
          )}
          <div className="relative">
            <select
            ...
            >
              <option value="">
                {statesLoading ? 'Carregando estados...' : 'Selecione um estado'}
              </option>
              {states.map((state) => (
